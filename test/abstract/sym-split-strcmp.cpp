...
int main() {
    { // equal
        auto lhs = sym::segmentation( Bound{ 0, 1 }, 'a', Bound{ 1, 2 }, 'b',
                                     Bound{ 2, 3 }, '\0', Bound{ 3, 4 } );
        auto rhs = sym::segmentation( Bound{ 0, 1 }, 'a', Bound{ 1, 2 }, 'b',
                                     Bound{ 2, 3 }, '\0', Bound{ 3, 4 } );
        assert( strcmp( &lhs, &rhs ) == 0 );
    }

    { // nonequal
        auto lhs = sym::segmentation( Bound{ 0, 1 }, 'a',  Bound{ 2, 3 }, '\0', Bound{ 3, 4 } );
        auto rhs = sym::segmentation( Bound{ 0, 1 }, 'a', Bound{ 1, 2 }, 'b',
                                     Bound{ 2, 3 }, '\0', Bound{ 3, 4 } );
        assert( strcmp( &lhs, &rhs ) < 0 );
    }
    ...
    { // maybe equal
        auto lhs = sym::segmentation( Bound{ 0, 1 }, 'a', Bound{ 2, 3 }, '\0', Bound{ 3, 4 } );
        auto rhs = sym::segmentation( Bound{ 0, 1 }, 'a', Bound{ 1, 3 }, '\0', Bound{ 3, 4 } );
