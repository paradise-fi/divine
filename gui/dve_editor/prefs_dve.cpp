...
const DvePreferences::HighlightInfo dveDefs[] = {
  ...
  {false, false, false, "darkmagenta", ""}, // hsTransition
  {false, false, false, "darkgreen", ""},   // hsKeyword
  {false, true, false, "darkgray", ""},     // hsComment
  {true, false, false, "", ""}              // hsMacro
};

DvePreferences::DvePreferences(QWidget * parent)
  : PreferencesPage(parent)
{
  ...
  new QTreeWidgetItem(tree_, QStringList(tr("Data Type")));
  new QTreeWidgetItem(tree_, QStringList(tr("Transition")));
  new QTreeWidgetItem(tree_, QStringList(tr("Keyword")));
  new QTreeWidgetItem(tree_, QStringList(tr("Comment")));
...
DvePreferences::~DvePreferences()
{
}

void DvePreferences::readSettings(void)
{
  ...
  for(int i = 0; i < tree_->topLevelItemCount(); ++i) {
    ...
    bool bold = s.value("bold", dveDefs[i].bold).toBool();
    bool italic = s.value("italic", dveDefs[i].italic).toBool();
    bool underline = s.value("underline", dveDefs[i].underline).toBool();
    ...
    str = s.value("foreground", dveDefs[i].foreground).toString();
    if(str.isEmpty()) {
    ...
    str = s.value("background", dveDefs[i].background).toString();
    if(str.isEmpty()) {
...
void DvePreferences::writeSettings(void)
{
  QSettings & s = sSettings();
...
void DvePreferences::onItemDoubleClicked(QTreeWidgetItem * item, int column)
{
  if(column < 4 || !item)
    return;
...
void DvePreferences::onItemChanged(QTreeWidgetItem * item, int column)
{
...
void DvePreferences::onCustomContextMenuRequested(const QPoint & pos)
{
...
void DvePreferences::onResetForeground(void)
{
...
void DvePreferences::onResetBackground(void)
{
  Q_ASSERT(target_);
