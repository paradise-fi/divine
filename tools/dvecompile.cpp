...
void dve_compiler::print_include(ostream & ostr)
{
...
void dve_compiler::print_test_header(ostream & ostr)
{
...
void dve_compiler::print_main(ostream & ostr)
{
  ostr << endl;
  ostr << "int main()" <<endl;
  ostr << "  {"<<endl;
...
void dve_compiler::print_state_t(ostream & ostr)
{
 ostr << "struct state_t" <<endl;
 ostr << " {" <<endl;
 ostr << "   char *ptr;"  <<endl;
 ostr << "   std::size_t size;" <<endl;
 ostr << " };" <<endl;
 ostr << endl;
}


void dve_compiler::print_new_state(ostream & ostr)
{
 ostr << "state_t new_state(const std::size_t size)" <<endl;
 ostr << " {" <<endl;
 ostr << "   state_t retval;"<<endl;
 ostr << "   retval.size = size;"<<endl;
 ostr << "   retval.ptr = new char[size];"<<endl;
 ostr << "   memset (retval.ptr,0,size);"<<endl;
 ostr << "   return retval;"<<endl;
 ostr<< "  };"<<endl;
 ostr << endl;
}

void dve_compiler::print_duplicate_state(ostream & ostr)
{
 ostr << "state_t duplicate_state(state_t state)" <<endl;
 ostr << " {" <<endl;
 ostr << "   state_t retval;"<<endl;
 ostr << "   retval.ptr = new char[state.size];"<<endl;
 ostr << "   memcpy (retval.ptr,state.ptr,state.size);"<<endl;
 ostr << "   retval.size = state.size;"<<endl;
 ostr << "   return retval;"<<endl;
 ostr<< "  };"<<endl;
 ostr << endl;
}
...
{
  string state_name = "input_states[j]";
  ...
  int succ_size = 0;
  int p_succ_size = 0;
  int max_p_succ_size = 0;
  string space = "   ";
  ostr << "extern "<< '"' << "C" << '"' << " __global__ void test_generator(int first_input_state, state_struct_t* input_states, size_t* results, state_struct_t* succs, int* add )" <<endl;
  ostr << " {"<<endl;
  ostr << space << "int state_size = " <<current_size<<";"<<endl;
  ostr << space << "int j = blockIdx.x + first_input_state;"<<endl;
  //ostr << space << "int i = threadIdx.x;"<<endl;
             ...
             iter_process_transition_map != transition_map.find(i)->second.end();iter_process_transition_map++)
         {
               ...
               iter_ext_transition_vector != iter_process_transition_map->second.end();iter_ext_transition_vector++)
           {
             ...
             if(iter_ext_transition_vector->synchronized)
             {
                ...
                {
                  ostr << space << "             ";
                  write_C(*iter_ext_transition_vector->first->get_sync_expr_list_item(s), ostr, "new_c_state");
                  ostr << " = ";
                  write_C(*iter_ext_transition_vector->second->get_sync_expr_list_item(s), ostr, "("+state_name+")");
                  ostr << ";" <<endl;
                }
             }
             else
             {
               if(iter_ext_transition_vector->first->get_sync_mode() == SYNC_EXCLAIM_BUFFER)
               {
                 ...
                 {
                    ostr << space << "             ";
                    ostr << "new_c_state." << get_symbol_table()->get_channel(iter_ext_transition_vector->first->get_channel_gid())->get_name()
                         << ".content[("+state_name+")."<< get_symbol_table()->get_channel(iter_ext_transition_vector->first->get_channel_gid())->get_name() 
                         << ".number_of_items].x" << s << " = ";
                    write_C(*iter_ext_transition_vector->first->get_sync_expr_list_item(s), ostr, "("+state_name+")");
                    ostr << ";" << endl;
                 }
                 ostr << space << "             ";
                 ostr << "new_c_state." << get_symbol_table()->get_channel(iter_ext_transition_vector->first->get_channel_gid())->get_name() 
               ...
               if(iter_ext_transition_vector->first->get_sync_mode() == SYNC_ASK_BUFFER)
               {
                 ...
                 ostr << "new_c_state." << get_symbol_table()->get_channel(iter_ext_transition_vector->first->get_channel_gid())->get_name() 
                      << ".number_of_items--;"<<endl;
                 ostr << space << "             ";
                 ostr << "for(size_int_t ii = 1 ; ii <= new_c_state." << get_symbol_table()->get_channel(iter_ext_transition_vector->first->get_channel_gid())->get_name() 
                      << ".number_of_items; ii++)"<<endl;
                 ostr << space << "               {" <<endl;
                 for(size_int_t s = 0;s < iter_ext_transition_vector->first->get_sync_expr_list_size();s++)
                 {
                   ostr << space << "                 ";
                   ostr <<  "new_c_state." << get_symbol_table()->get_channel(iter_ext_transition_vector->first->get_channel_gid())->get_name() << ".content[ii-1].x" << s
                        << " = ("+state_name+")." << get_symbol_table()->get_channel(iter_ext_transition_vector->first->get_channel_gid())->get_name() << ".content[ii].x" << s << ";" <<endl;
                   ostr << space << "                 ";
                   ostr <<  "new_c_state." << get_symbol_table()->get_channel(iter_ext_transition_vector->first->get_channel_gid())->get_name() << ".content[ii].x" << s
                        << " = 0;" <<endl;
                 }
                 ostr << space << "               }" <<endl;
               }
             ...
             ostr << space << "             new_c_state."<<get_symbol_table()->get_process(iter_ext_transition_vector->first->get_process_gid())->get_name()
             ...
             {
                ostr << space << "             ";
                write_C(*iter_ext_transition_vector->first->get_effect(e), ostr, "new_c_state");
                ostr <<";"<< endl;
             }
             if(iter_ext_transition_vector->synchronized) //second transiton effect
             {
               ostr << space << "             new_c_state."<<get_symbol_table()->get_process(iter_ext_transition_vector->second->get_process_gid())->get_name()
               ...
               {
                  ostr << space << "             ";
                  write_C(*iter_ext_transition_vector->second->get_effect(e), ostr, "new_c_state");
                  ostr <<";"<< endl;
               }
             }
             if(sytem_with_property) //change of the property process state
             {
               ostr << space << "             new_c_state."<<get_symbol_table()->get_process(iter_ext_transition_vector->property->get_process_gid())->get_name()
                    << ".state = "<< iter_ext_transition_vector->property->get_state2_lid()<< ";" <<endl;
             }
             //ostr << space << "             succs[number_of_input_states*"<<max_number_of_transitions*function_id<<"+j*succ_size+i] = new_c_state;"<<endl;
             ostr << space << "             int index = atomicAdd(add,1);"<<endl;
             ostr << space << "             succs[index] = new_c_state;"<<endl;
             ostr << space << "             results[index] = get_hash((unsigned char*) &new_c_state,state_size,EXPLICIT_STORAGE_HASH_SEED);"<<endl;
             //ostr << space << "             results[number_of_input_states*"<<max_number_of_transitions*function_id
                  //<<"+j*succ_size+i] = get_hash((unsigned char*) &new_c_state,state_size,EXPLICIT_STORAGE_HASH_SEED);"<<endl;
             ostr << space << "          }" <<endl;/*
             ostr << space << "          else{" <<endl;
             ostr << space << "             results[number_of_input_states*"<<max_number_of_transitions*function_id<<"+j*succ_size+i] = 0;"<<endl;
             ostr << space << "          }" <<endl;*/
           }
           ostr << space << "          break;" <<endl;
           ostr << space << "      }"<<endl; //end of case
           if(max_p_succ_size < p_succ_size)
             max_p_succ_size = p_succ_size;
         }
         ostr <<"     }" <<endl; //end of switch
         succ_size = succ_size + max_p_succ_size;
     }
   }
...
ostr <<" int succ_size = "<< succ_size <<";"<<endl;
ostr<< endl;

// is_accepting
  state_name = "state";
  ostr << "__host__ __device__  int is_accepting(state_struct_t state)" <<endl; // only Buchi acceptance condition
  ostr << " {"<<endl;
  if(sytem_with_property)
  {
...
void dve_compiler::print_test_generator(ostream & ostr) // !!!! self synchronization not fixed !!!!!
{
  string state_name = "input_states[j]";

  int max_number_of_transitions = 30;
  ...
  ostr << endl;
  div_t number_of_funtion = div(number_of_transitions,max_number_of_transitions);
  ostr << "int number_of_function = " << number_of_funtion.quot + 1 << ";" << endl;
  ostr<< endl;
  for(int i=0; i<number_of_funtion.quot; i++)
   ostr << "int succ_size_"<<i<<"  = " << max_number_of_transitions << ";" << endl;
  if(number_of_funtion.rem)
    ostr << "int succ_size_"<<number_of_funtion.quot<<"  = " << number_of_funtion.rem << ";" << endl;
  ostr << endl;
 ...
 for(iter_transition_map = transition_map.begin(); iter_transition_map != transition_map.end(); iter_transition_map++)
  {
         ...
         iter_process_transition_map != iter_transition_map->second.end();iter_process_transition_map++)
     {
       for(iter_ext_transition_vector = iter_process_transition_map->second.begin();
           iter_ext_transition_vector != iter_process_transition_map->second.end();iter_ext_transition_vector++)
       {
             if(label % max_number_of_transitions == 0)
             {
               ostr << "extern "<< '"' << "C" << '"' << " __global__ void test_generator_"<<function_id 
                    <<"(int number_of_input_states, state_struct_t* input_states, size_t* results, state_struct_t* succs)"<<endl;
                    //<<"(int number_of_input_states, state_struct_t* input_states, size_t* results, state_struct_t* succs, int* add )"<<endl;
               ostr << " {"<<endl;
               if(function_id < number_of_funtion.quot )
                  ostr << space << "int succ_size  = " << max_number_of_transitions << ";" << endl;
               else
                  ostr << space << "int succ_size  = " << number_of_funtion.rem << ";" << endl;
               ostr << space << "int state_size = " << current_size << ";" << endl;
               ostr << space << "int i = blockIdx.x;" <<endl;
               ostr << space << "int j = threadIdx.x;" <<endl;
               ostr << space << "switch(i+"<<max_number_of_transitions*function_id<<")" <<endl;
               ostr << space << "  {"<<endl;
             }


             ostr << space << " case " << label <<" :{ " << endl;
             ostr << space << "           if( ";
             ostr << state_name << "." << get_symbol_table()->get_process(iter_ext_transition_vector->first->get_process_gid())->get_name()
                    << ".state == "<< iter_ext_transition_vector->first->get_state1_lid();
             if(iter_ext_transition_vector->first->get_guard()!= 0 )
             {
               ostr << " && ( ";
               write_C(*iter_ext_transition_vector->first->get_guard(), ostr, state_name);
             ...
             //ostr << space << "             results[index] = get_hash((unsigned char*) &new_c_state,state_size,EXPLICIT_STORAGE_HASH_SEED);"<<endl;
             ostr << space << "             results[number_of_input_states*"<<max_number_of_transitions*function_id
                  <<"+j*succ_size+i] = 1;"<<endl;//get_hash((unsigned char*) &new_c_state,state_size,EXPLICIT_STORAGE_HASH_SEED);"<<endl;
             ostr << space << "          }" <<endl;
             ostr << space << "          else{" <<endl;
             ostr << space << "             results[number_of_input_states*"<<max_number_of_transitions*function_id<<"+j*succ_size+i] = 0;"<<endl;
             ostr << space << "          }" <<endl;
             ...
             if(label % max_number_of_transitions == max_number_of_transitions-1)
             {
                ostr << space << "  }" <<endl;
                ostr << " };" <<endl;
                function_id++;
   ...
   for(int i = 0; i < 67; i++)
   {
     ostr<< space<<"dim3 threads_"<<i<<"(number_of_input_states);"<<endl;
     ostr<< space<<"dim3 grid_"<<i<<"(succ_size_"<<i<<");"<<endl;
     ostr<< space<<"test_generator_"<<i<<"<<< grid_"<<i<<", threads_"<<i<<">>>(number_of_input_states,input_states,results,succs,add);"<<endl;
   }*/

   // TO DO commited states and deadlocks states
  if(number_of_funtion.rem)
  {
   ostr << space << "  }" <<endl;
   ostr << " };" <<endl;
...
void dve_compiler::print_DiVinE2_generator(ostream & ostr)
{
  map<size_int_t,map<size_int_t,vector<ext_transition_t> > > transition_map;
  map<size_int_t,vector<dve_transition_t*> > channel_map;
