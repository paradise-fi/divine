 ...
 *
 * DIVINE compact is compact explicit representation of state-space.
 * It can be generated by algorithm compact, even in distributed way, all
 * parameters that can be passed to divine metrics can be also passed to
 * compact, for example option to compress state-space when exploring it
 *    $ divine compact <model> [ --compression=tree ... ]
 * If compact state space is generated in distributed environment it is
 * expected that output file will be on shared filesystem.
 *
 * Each state in compact representation is assigned natural number, those
 ...
 *
 * Compact V1 header is 128B long, with fixed format, see struct Header.
 *
 * At runtime, compact file (with .dcess extension -- DiVinE Compact Explicit
 * State Space) is mapped to memory to allow easy access of it.
 * To do this object of type Compact is used. This object can be copied,
 * but it is shallow copy. File is automatically unmapped when object
 * (all copies) come out of scope. Compact file is opened either by
 * giving path in constructor or by calling open( string path ) method
 * on object.
 *
 * Writing to Compact data will result in SIGSEGV or similar if flags
...
struct Header {
    ...
    static Header *ptr( void *p ) {
        ...
        if ( std::memcmp( ptr->magic, MAGIC, MAGIC_LENGTH ) != 0 ) {
            std::cerr << "ERROR: Invalid DiVinE Compact format" << std::endl;
            std::abort();
        }
...
// Compact V1 runtime representation
struct Compact {
    ...
    Compact( std::string path, int openmode = O_RDONLY, int mmapProt = PROT_READ ) {
